...
SET_Y2RANGE="[$POW_LIMIT:$POW_LIMIT+(3.5e-07-$POW_LIMIT)*5]"
...
set xrange [0:*]; set xtics 1, 17*30*10 rotate by 45 right; set xtics add ("GENESIS" 0) ( "N+1=$(($DIFF_N_SIZE+1))" $(($DIFF_N_SIZE+1)) );
set ylabel "Nonce";
set yrange [8.5e+08*-1:8.5e+08*(5+1)]; set ytics 8.5e+08; set format y '%.3g'; set ytics nomirror;
set y2label "Difficulty" tc rgb "red";
set y2range $SET_Y2RANGE; set format y2 '%.3g'; set y2tics $POW_LIMIT, (3.5e-07-$POW_LIMIT)/1; set y2tics add ($POW_LIMIT);
set grid xtics ytics mxtics mytics;
set key top left; set key box opaque;
plot \
"$NONCE_FILE_NAME" using 0:3 axis x1y1 w p title "Nonce" pt 7 ps 0.05 lc rgb "black", \
